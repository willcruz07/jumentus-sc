...
export const useMatches: UseBoundStore<StoreApi<TState & TActions>> = create<TState & TActions>(
  (set) => ({
    ...
    createMatch: async (data) => {
      ...
      try {
        ...
        await addDoc(docRef, {
          ...
          playerOfTheDay: null,
          worstPlayerOfTheDay: null,
          playersScoreOnTheDay: [...data.players.goalKeepers, ...data.players.players].map(
            (player) => ({
              fullName: player,
    ...
    async restartMatchDay() {
      const details = useMatches.getState();
      setLoadingState(useMatches, 'startMatch', true);

      try {
        const docRef = doc(dbFirestore, FIREBASE.COLLECTIONS.MATCHES, details?.matchId ?? '');
    ...
    restartMatch: async () => {
      ...
      try {
        const pauseDuration = dayjs().diff(dayjs(details.matchInProgress?.pausedTime), 'second');

        const newStartTime = dayjs(details.matchInProgress?.startTime).add(pauseDuration, 'second');
      ...
      try {
        ...
        if (details?.matchInProgress) {
          await updateDoc(docRef, {
            matchInProgress: {
              ...details.matchInProgress,
              playersWhoScored: [...(details.matchInProgress?.playersWhoScored ?? []), name],
            },
          });
        }
      } catch (error) {
    ...
    setPlayersScoreOnTheDay: async (player) => {
      ...
      try {
        const playerScore = details.playersScoreOnTheDay.find(
          (findPlayer) => findPlayer.fullName.toLowerCase() === player.fullName.toLowerCase(),
        );
    ...
    setFinishMatch: async () => {
      ...
      const calculateTeamScores = (details: any, teamIndex: number) => {
        ...
        if (details.matchInProgress.goals[0] === details.matchInProgress.goals[1]) {
          draw += 1;
        }
        if (
              ...
              matchInProgress: data?.matchInProgress
                ? {
                    ...
                    startTime: data?.matchInProgress?.startTime
                      ? dayjs((data?.matchInProgress?.startTime as any)?.seconds * 1000).toDate()
                      : null,
                    pausedTime: data?.matchInProgress?.pausedTime
                      ? dayjs((data?.matchInProgress?.pausedTime as any)?.seconds * 1000).toDate()
                      : null,
                  }
                : null,
            });
