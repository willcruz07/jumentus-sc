...
export const usePlayers: UseBoundStore<StoreApi<TState & TActions>> = create<TState & TActions>(
  (set) => ({
    ...
    setPlayers: async (player) => {
      ...
      try {
        const docRef = doc(dbFirestore, FIREBASE.COLLECTIONS.PLAYERS, player.id ?? '');

        await updateDoc(docRef, {
          fullName: player.fullName,
          name: player.name,
          matches: !playerSelected ? player.matches : playerSelected.matches + player.matches,
          assists: !playerSelected ? player.assists : playerSelected.assists + player.assists,
          goals: !playerSelected ? player.goals : playerSelected.goals + player.goals,
          saves: !playerSelected ? player.saves : playerSelected.saves + player.saves,
          tackles: !playerSelected ? player.tackles : playerSelected.tackles + player.tackles,
        });
      } catch (error) {
        console.error(error);
      } finally {
          ...
          playersScore.forEach((player) => {
            ...
            if (dataIndex !== -1) {
              ...
              data[dataIndex].manOfTheMatch +=
                bestPlayer?.fullName?.toLowerCase() === player?.fullName?.toLowerCase() ? 1 : 0;
              data[dataIndex].shitOfTheMatch +=
                shitPlayer?.fullName?.toLowerCase() === player?.fullName?.toLowerCase() ? 1 : 0;
            } else {
