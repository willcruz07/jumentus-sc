...
export function MatchCreate() {
  ...
  const allPlayers = usePlayers((state) => state.allPlayers);
  const startListenerAllPlayers = usePlayers((state) => state.startListenerAllPlayers);

  const loading = useMatches((state) => state.loading);
  const createMatch = useMatches((state) => state.createMatch);
    ...
    lines.forEach((line: string) => {
      if (line.includes('📅')) {
        ...
        if (dateMatch) {
          ...
          date = new Date(Number(baseDate[2]), Number(baseDate[1]) - 1, Number(baseDate[0]));
        }
      }
      ...
      if (inPlayersSection && line.match(/^\d+/)) {
        ...
        if (nameMatch) {
          ...
          if (nameMatch[2] === '🔁' || nameMatch[2] === '🔃' || nameMatch[2] === '🔄') {
  ...
  const divideTeams = (playersList: Array<string>): ITeams => {
    ...
    if (playersSelected && playersSelected?.goalKeepers?.length > 2) {
      ...
      const shuffledGoalkeepers = shuffleArray([...playersSelected.goalKeepers]);
  ...
  const handleCustomTeam = (player: string, keyTeam: keyof ITeams) => {
    ...
    setTeams((pS) => {
      ...
      const updatedTeams = (Object.keys(pS) as Array<keyof ITeams>).reduce((acc, teamKey) => {
        if (teamKey === keyTeam) {
    ...
    <MainContainer>
      ...
      <div className="mt-8 flex w-full flex-col items-center">
        <div className="w-full max-w-prose">
          <TextArea value={playerList} label="Lista de jogadores" onChange={setPlayerList} />

          <Button className="mt-6" onClick={handleCreatePlayersList} label="Listar jogadores" />
        </div>

        {playersSelected && (
          <div className="w-full max-w-prose">
            <div>
              <h1 className="mt-8 mb-2 text-2xl text-gray-300">{'Jogadores'}</h1>
              <div className="mt-4 flex flex-col gap-3">
                {playersSelected?.players?.map((player) => (
                  <ContainerPlayer
                    className="min-h-12"
                    onClick={(value) => handleCustomTeam(player, value as keyof ITeams)}
                    key={player}
                    name={player}
                    teams={teams}
                    fullName={player}
              ...
              <h1 className="mt-8 mb-2 font-sans text-2xl text-gray-300">{'Goleiros'}</h1>
        ...
        {playersSelected?.players && (
          <div className="w-full max-w-prose">
            <Button
              className="mt-10"
              onClick={() =>
                playersSelected?.players && setTeams(divideTeams(playersSelected?.players))
              }
              ...
              <ContainerTeams key={index} name={`Time - ${index + 1}`}>
                {(teams as any)[teamKey].map((player: string) => {
                  const foundPlayer = allPlayers.find((p) => {
                    return p.fullName.trim().toLocaleLowerCase() === player.trim().toLowerCase();
                  });
