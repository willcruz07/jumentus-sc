      ...
      <Dialog modal onOpenChange={(open) => setModalOpen(open)} open={modalOpen}>
        <DialogContent className="border-slate-700 bg-slate-900 [&>button:last-child]:hidden">
          ...
          <div className="flex flex-col gap-4">
            <Button label="Time - 1" onClick={() => handleConfirmTeam('team_1')} />
            <Button label="Time - 2" onClick={() => handleConfirmTeam('team_2')} />
            <Button label="Time - 3" onClick={() => handleConfirmTeam('team_3')} />
          </div>
        </DialogContent>
      </Dialog>
    </>
